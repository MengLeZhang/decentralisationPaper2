...
match.form <- I(country == 'Scotland') ~ total.pop + total.area + dist_nearest04

set.seed(123) ## genetic matchhing is a random process so we must set seed
match.res <- 
  matchit(match.form,
          data = unmatch.tab %>% dplyr::select(ttwa:total.area, dist_nearest04),
          method = 'genetic')
...
form2 <- ~ country + scale(total.pop) + scale(total.area) + dist_nearest04 ## We omit the I(year - 2004) variable since last 
...
sub.reg2 <- lm(form2 %>% update(suburb ~ .), reg.match, weights = weights)
...
#          dep.var.labels = c('RCI', 'Suburbanisation', 'DI'),
          ...
          column.labels = c('RCI', 'Suburbanisation', 'DI'),
...
summary.tab <-
  ...
  summarise(
    ...
    change_suburb.inc = suburb.inc[year >= 2015] - suburb.inc[year == 2004],
    change_suburb.HH = suburb.HH[year >= 2015] - suburb.HH[year == 2004],
...
compare.tab %>% 
  filter(year == '2004') %>% 
  dplyr::select(starts_with('dist_nearest.'), starts_with('suburb'), starts_with('di.')) %>%
...
We can also fix the amenities that vary over time

```{r fixed}

fixed_crime.reg <- lmer(form1 %>% update(crime_fixed ~ .), reg.unmatch)
fixed_crime.reg2 <- lmer(form2 %>% update(crime_fixed ~ .), reg.match)
fixed_geo.reg <- lmer(form1 %>% update(geo_fixed ~ .), reg.unmatch)
fixed_geo.reg2 <- lmer(form2 %>% update(geo_fixed ~ .), reg.match)

fixed_pm25.reg <- lmer(form1 %>% update(pm25_fixed ~ .), reg.unmatch)
fixed_pm25.reg2 <- lmer(form2 %>% update(pm25_fixed ~ .), reg.match)

fixed_reg.list <- 
  list(
    fixed_crime.reg, fixed_crime.reg2,
    fixed_geo.reg, fixed_geo.reg2,
    fixed_pm25.reg, fixed_pm25.reg2
  )

stargazer(fixed_reg.list, type = 'text',
          title = 'Random effect model results',
          dep.var.caption = 'Inequality index',
          dep.var.labels = c('Crime', 'Geo. access', 'Air pollution'),
          covariate.labels = c('Year (2004 = 0)', 'Scotland', 'Year:Scotland'),
          column.labels = rep(c('All', 'Matched'), 3),
          omit = c('scale', 'dist_nearest04'),
          no.space = T,
