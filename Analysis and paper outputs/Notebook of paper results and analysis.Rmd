...
reg.match <-
  reg.match %>% filter(year %in% c('2019', '2016'))
...
summary.tab <-
  ...
  group_by(ttwa) %>%
  summarise(
    change_rci.inc = dist_nearest.inc[year >= 2016] - dist_nearest.inc[year == 2004],
    change_rci.HH = dist_nearest.HH[year >= 2016] - dist_nearest.HH[year == 2004],

    change_di.inc = di.inc[year >= 2016] - di.inc[year == 2004],
    change_di.HH = di.HH[year >= 2016] - di.HH[year == 2004]
