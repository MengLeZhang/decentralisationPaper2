...
NOTE: We also deliberately filter to cover IMD years up to 2016 to omit 
English 2019 IMD until such time that a near in time Scottish IMD is available.
...
rci.gg <- 
  ...
  geom_line(size = 1) +
  theme(plot.title = title_size) +
  ggtitle('(i) Centralisation (RCI)') +
  ylab(NULL) 
...
grid.arrange(mylegend, seg.grid, nrow=2,heights=c(1, 12),
             top = 'Change in segregation indicies (Error bars for annual comparison)')
...
grid.arrange(mylegend, gfx, nrow=2,heights=c(1, 12),
             top = 'Change in inequality indicies (Error bars for annual comparison)')
...
reg.match <-
  reg.match %>% filter(year %in% c('2015', '2016')) ## This line determines which years are compared
...
#          keep.stat = c('n'),
          notes = 'Some parameter estimates are omitted'
)
...
summary.tab <-
  ...
  group_by(ttwa) %>%
  summarise(
    change_rci.inc = dist_nearest.inc[year %in% 2015:2016] - dist_nearest.inc[year == 2004],
    change_rci.HH = dist_nearest.HH[year %in% 2015:2016] - dist_nearest.HH[year == 2004],

    change_di.inc = di.inc[year %in% 2015:2016] - di.inc[year == 2004],
    change_di.HH = di.HH[year %in% 2015:2016] - di.HH[year == 2004]
...
#          column.labels = rep(c('All', 'Matched'), 5),
          ...
          no.space = T,
          notes = 'Note: Scottish crime results only available from 2006 onwards; some parameter estimates are omitted',
...
stargazer(fixed_reg.list, type = 'text',
         ...
         add.lines =  list( c('Fixed values at 2004', rep(c('F', 'T'), 3)) ),
          omit = c('scale', 'dist_nearest04'),
          no.space = T,
          notes = 'Note: Scottish crime results uses 2006 SIMD values; some parameter estimates are omitted',
